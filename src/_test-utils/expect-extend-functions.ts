...
expect.extend({
	toBeEquivalentToBTWindow: (actual: chrome.windows.Window, expected: BT.Window) => {
		...
		if (focusPass && urlsLengthPass && urlsPass && visiblePass) {
			...
			if (!visiblePass) {
				...
				message = `expected focused to be ${expected.focused}, but instead it was ${actual.focused}`;
