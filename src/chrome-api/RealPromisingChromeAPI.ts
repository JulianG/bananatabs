...
export default class RealPromisingChromeAPI implements ChromeAPI {
	...
	constructor() {
		...
		this.windows = {
			...
			create: promisify<chrome.windows.Window | undefined>(chrome.windows.create),
		...
		this.system = {
			display: {
				getInfo: promisify<chrome.system.display.DisplayUnitInfo[]>(chrome.system.display.getInfo)
			}
		};
